...
**Coverage: 16/18 workloads run on our backend (89%).**  *(§42: uint32 buffers
now report U32 (not S32) at the host boundary → monte_carlo PASS, bit-exact vs
CPU; brax reset+step LOWERS (past the ex-"JAX threefry ui32/i32" wall — it was
OUR dtype bug). §39: direct N-D convolution — cnn now PASS via OP_CONV. Remaining
fails: fft (complex dtype), brax_step (now blocked on the DEVICE-op gap
`reduce(and)`, not lowering).)*
