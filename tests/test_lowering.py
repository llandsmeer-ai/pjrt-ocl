...
def test_golden_layout_exact_bytes():
    ...
    # p6/p7 (§33 R2c matmul epilogue) = 0 for non-matmul tasks.
    t0 = struct.unpack_from("<12I", blob, pos)
    assert t0 == (S.TILE_EW, 3, 0, 1, S.EW_ADD, 3, 0, 0, 0, 0, 0, 0)
    pos += 48 * n_tasks  # skip past both tasks (task_t is 48B now)
    ...
    assert len(blob) == tensor_len + 16 + 48 * n_tasks + 16 * n_lanes + 32 * n_entries
...
def test_golden_layout_jax_lowered_add():
    ...
    pos += 16
    # task_t is 12 u32 words (p4/p5 = MMA view offsets; p6/p7 = §33 R2c matmul
    # epilogue fields; all 0 for this EW task).
    assert struct.unpack_from("<12I", blob, pos) == (S.TILE_EW, 2, 0, 1,
                                                     S.EW_ADD, 8, 0, 0, 0, 0,
                                                     0, 0)
    pos += 48 * n_tasks + 16 * n_lanes
