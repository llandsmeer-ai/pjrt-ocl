...
int main(void){
    ...
    struct { const char*name,*opts; } steps[] = {
      /* winning region: MORE warps per tile => thinner f32 accumulator/thread =>
       * higher occupancy; f16 2x rate keeps it off the compute wall. acc/thr in [] */
      {"F16 128x128 W4x4 BK16 NBUF2 (512t)",  "-DHP=1 -DTM=128 -DTN=128 -DBK=16 -DNBUF=2 -DWM=4 -DWN=4"},
      {"F16 256x128 W8x4 BK16 NBUF2 (1024t)", "-DHP=1 -DTM=256 -DTN=128 -DBK=16 -DNBUF=2 -DWM=8 -DWN=4"},
      {"F16 256x128 W8x2 BK16 NBUF2 (512t)",  "-DHP=1 -DTM=256 -DTN=128 -DBK=16 -DNBUF=2 -DWM=8 -DWN=2"},
      {"F16 256x128 W16x2 BK16 NBUF2 (1024t)","-DHP=1 -DTM=256 -DTN=128 -DBK=16 -DNBUF=2 -DWM=16 -DWN=2"},
      {"F16 256x256 W16x2 BK16 NBUF2 (1024t)","-DHP=1 -DTM=256 -DTN=256 -DBK=16 -DNBUF=2 -DWM=16 -DWN=2"},
      {"F16 256x256 W8x4 BK16 NBUF2 (1024t)", "-DHP=1 -DTM=256 -DTN=256 -DBK=16 -DNBUF=2 -DWM=8 -DWN=4"},
      {"F16 128x256 W4x8 BK16 NBUF2 (1024t)", "-DHP=1 -DTM=128 -DTN=256 -DBK=16 -DNBUF=2 -DWM=4 -DWN=8"},
      {"F16 384x128 W8x4 BK16 NBUF2 (1024t)", "-DHP=1 -DTM=384 -DTN=128 -DBK=16 -DNBUF=2 -DWM=8 -DWN=4"},
      {"F16 256x128 W8x4 BK16 NBUF3 (1024t)", "-DHP=1 -DTM=256 -DTN=128 -DBK=16 -DNBUF=3 -DWM=8 -DWN=4"},
      {"F16 256x128 W8x4 BK32 NBUF2 (1024t)", "-DHP=1 -DTM=256 -DTN=128 -DBK=32 -DNBUF=2 -DWM=8 -DWN=4"},
      /* bf16 (wider exponent) at the two best shapes */
      {"BF16 256x128 W8x4 BK16 NBUF2 (1024t)","-DHP=2 -DTM=256 -DTN=128 -DBK=16 -DNBUF=2 -DWM=8 -DWN=4"},
      {"BF16 128x128 W4x4 BK16 NBUF2 (512t)", "-DHP=2 -DTM=128 -DTN=128 -DBK=16 -DNBUF=2 -DWM=4 -DWN=4"},
    };
    int ns=sizeof steps/sizeof steps[0];
    const char*only=getenv("ONLY");

    if(envi("WARMUP",1)){ cl_kernel wk=build("-DHP=1 -DTM=128 -DTN=128 -DBK=16 -DNBUF=2");
    ...
    printf("\nref: tf32 ceiling ~57 TF/s (§36); cuBLAS f32/tf32 116-133 TF/s\n");
    return 0;
}
